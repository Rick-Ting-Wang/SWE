-- Switch to the target database (must create the database first before executing this script)
USE komodo;

-- 1. Users table (stores all user info for all roles)
CREATE TABLE users
(
...
-- 3. Organization members table (user-organization mapping, many-to-many)
...
(
    ...
    UNIQUE KEY unique_org_user (org_id, user_id), -- Only one identity for a user in an organization
    INDEX         idx_role (role)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
-- 5. Class enrollments table (students and classes mapping)
...
(
    ...
    UNIQUE KEY unique_class_student (class_id, student_id), -- A student enrolls a class only once
...
-- 7. Program enrollments table (mapping of users/orgs to programs, supports both)
CREATE TABLE program_enrollments
(
    enrollment_id   INT PRIMARY KEY AUTO_INCREMENT,
    program_id      INT  NOT NULL,
    user_id         INT, -- Personal signup (can be NULL)
    ...
    FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE,
    FOREIGN KEY (org_id) REFERENCES organizations (org_id) ON DELETE CASCADE,
    CHECK (user_id IS NOT NULL OR org_id IS NOT NULL), -- At least one of user/org not NULL
...
-- 8. Activities table (concrete activities under programs/classes)
...
-- 9. Content library table (resources such as articles/photos/teaching materials)
...
(
    ...
    content_data TEXT,
    created_by   INT          NOT NULL,
    org_id       INT, -- Optional related org (can be NULL)
...
-- 12. Assessments table (teachers grade/feedback for submissions)
...
(
    ...
    target_id   INT  NOT NULL, -- ID of the related target (e.g., submission_id, content_id)
    note_text   TEXT NOT NULL,
    created_at  TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (teacher_id) REFERENCES users (user_id) ON DELETE CASCADE,
    INDEX       idx_teacher (teacher_id),
    INDEX       idx_target (target_type, target_id) -- Joint index for quickly locating target notes
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
(
    ...
    is_public    BOOLEAN DEFAULT TRUE, -- Profile is public or not
    FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE,
...
-- 16. Creative canvas table (stores user's creative assets for a program)
CREATE TABLE creative_canvas
(
    canvas_id  INT PRIMARY KEY AUTO_INCREMENT,
    user_id    INT NOT NULL,
    program_id INT NOT NULL,
    assets     JSON, -- Store creative assets (e.g. JSON-formatted configuration)
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE,
    FOREIGN KEY (program_id) REFERENCES programs (program_id) ON DELETE CASCADE,
...
-- 17. Access logs table (user operation logs for audit/trace)
CREATE TABLE access_logs
(
    log_id      BIGINT PRIMARY KEY AUTO_INCREMENT, -- Use BIGINT to avoid overflow for large log volume
    user_id     INT, -- NULL for anonymous actions
    ...
    FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE SET NULL, -- Logs retained after user deletion, user_id set to NULL
    INDEX       idx_user (user_id),
...
-- 18. Business analytics table (stores metric/statistics data)
CREATE TABLE business_analytics
(
    analytics_id INT PRIMARY KEY AUTO_INCREMENT,
    metric_type  VARCHAR(100) NOT NULL, -- Metric type (e.g. "daily_active_users", "program_enrollments")
    metric_value DECIMAL(15, 2), -- Metric value (supports decimal, e.g. conversion rate 23.5%)
    metric_data  JSON, -- Extended metric data (e.g. active users split by role)
